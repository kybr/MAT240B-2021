...
const int CLIPSIZE = 2048;
const int HOPSIZE = CLIPSIZE / 2;
const int FFTSIZE = CLIPSIZE * 4;
...
void load(std::vector<float> &input, const char *filePath) {
  ...
  float *pSampleData = drwav_open_file_and_read_pcm_frames_f32(
      filePath, &channels, &sampleRate, &totalPCMFrameCount, nullptr);
...
struct Grain {
  ...
  Grain(const std::vector<float> &audio) : audio(audio) {}
  ...
  //
  Counter<CLIPSIZE> index;
  float operator()() {
    return audio[begin + index()] * (1 - cos(2 * M_PI * i / (CLIPSIZE))) / 2;
  }
};

struct GrainPlayer {
  const std::vector<Grain> &grain;
  GrainPlayer(const std::vector<Grain> &grain) : grain(grain) {}
